...
(async()=>{
 ...
 const code=incoming||storedCode;if(incoming)localStorage.setItem(ACCESS_KEY,incoming);
 ...
 if(!navigator.onLine){if(offlineValid())openGame('📡 Modo offline · acceso validado previamente en este dispositivo.');else state.textContent='⚠️ Necesitás conectarte una vez desde Mi Biblioteca para validar este dispositivo.';return}
 ...
 try{
   if(!code)throw new Error('Abrí Torre de América desde “Mis juegos” en tu Biblioteca PasaloChévere.');
   ...
   const {data:{session}}=await sb.auth.getSession();if(!session?.user?.email)throw new Error('Ingresá desde Mi Biblioteca para validar tu cuenta.');
 ...
 }catch(e){state.textContent='⚠️ '+(e?.message||'Acceso no válido.')+' Volvé a tu biblioteca y abrí Torre de América desde allí.'}
})();
